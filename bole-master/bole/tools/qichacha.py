...
def tianyancha(url):
    ...
    try:
        ...
        if tel:
            ...
            try:
                ...
                www='暂无'
        if not tel:
            tel=html.xpath('//div[@class="content"]/div[3]/span[2]/span[1]/text()')
            email=html.xpath('//div[@class="content"]/div[4]/span[2]/a/text()')
            try:
                www=html.xpath('//div[@class="content"]/div[4]/span[4]')[0].xpath('string(.)')
            except:
                www='暂无'
        #公司地址
        try:
        ...
        print(company,tel,email,www,addr,delegate,login_money,real_money,status,login_time,Registration,org_number,
    ...
    try:
        ...
        # 设置需要插入的行的名称
        insert_sql = """
            insert into qichacha(url,company,tel,email,www,addr,delegate,login_money,real_money,status,login_time,Registration,org_number,
                  taxpayer_number,credit_number,company_type,profession,approved_date,registration_org,region,english_name,
                  old_name,business_mode,staff_size,business_date,business_scope,partner_info)
             VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """
        cursor.execute(insert_sql, (
        url,company, tel, email, www, addr, delegate, login_money, real_money, status, login_time, Registration, org_number,
        taxpayer_number, credit_number, company_type, profession, approved_date, registration_org, region, english_name,
        old_name, business_mode, staff_size, business_date, business_scope, partner_info))
        # 完成插入后提交
        conn.commit()
        ...
        print(e)
# url = 'http://www.qichacha.com/firm_279920d8d22c726d0b154865070cbef5.html'
# tianyancha(url)
def get_url():
    for i in range(20,501):
        url='http://www.qichacha.com/g_AH_{}.html'.format(i)
        headers={
            'Accept':'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
            'Accept-Encoding':'gzip, deflate, sdch',
            'Accept-Language':'zh-CN,zh;q=0.8',
        ...
        # print(a)
        html = etree.HTML(a)
        numbers=html.xpath('//*[@id="searchlist"]/a/@href')
        for number in numbers:
            info_url='http://www.qichacha.com{}'.format(number)
            tianyancha(info_url)
